// ISO 3166-1 alpha-2 codes + common English names + E.164 calling codes, for
// the shipping-address country dropdown and phone-number dial-code select
// (see ShopSection.tsx). Sorted alphabetically by name.
export interface Country {
  code: string;
  name: string;
  // E.164 calling code, e.g. "+44" -- countries that share the North
  // American Numbering Plan (US, Canada, and the Caribbean entries) each
  // keep their own distinct code here (e.g. "+1242" for Bahamas) rather
  // than the bare "+1", so the dial-code select unambiguously identifies one
  // country per entry.
  dialCode: string;
}
